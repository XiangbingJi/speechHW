...
double do_viterbi(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    double curHighest=g_zeroLogProb;
   ...
   // ** * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * **
    ...
    for (int frmIdx = 0; frmIdx <= frmCnt; ++frmIdx)
        ...
        double threshLogProb=curHighest-beamLogProb;
        curHighest=g_zeroLogProb;
        ...
        // computing the k-th highest prob
        ...
        vector<double> v;
        v.clear();
        if (cellCnt<beamStateCnt || beamStateCnt==0){kth_highest=g_zeroLogProb;}
        else{
        ...
        v.push_back(curLogProb);
        }
        std::sort (v.begin(), v.end()); 
        std::reverse(v.begin(),v.end());
        kth_highest=v[beamStateCnt-1];  
        }      
        ...
        while (((curState = curFrame.get_next_state())) >= 0)
        {
            ...
            if (curCell.get_log_prob() < threshLogProb || curCell.get_log_prob()<kth_highest)
                 continue;
            ...
            for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx)
            {
                ...
                int lastword=arc.get_word();
                int parentIdx=curCell.get_node_index();
                int dstWordTreeIdx;
                double newProb;
                FrameCell* destCell;
                FrameCell* skipCell;

                if (lastword!=0) {dstWordTreeIdx=wordTree.insert_node(parentIdx,lastword);}
                else {dstWordTreeIdx=parentIdx;}

                //  Fill in update of destination cell here.

                if (hasGmm==false)
                {
                    //if (curFrame.has_cell(dstState)==false) {
                    skipCell=&(curFrame.insert_cell(dstState));
                    //}
                    
                    //skipCell=&(curFrame.get_cell_by_state(dstState));

                    newProb=arc.get_log_prob()+curCell.get_log_prob();

                    if (skipCell->get_log_prob()<newProb){skipCell->assign(newProb,dstWordTreeIdx);}
                }
                ...
                if (hasGmm==true)
                {
                    ...
                    newProb=arc.get_log_prob()+gmmProbs(frmIdx,arc.get_gmm())*acousWgt+curCell.get_log_prob();

                    if (newProb>curHighest){curHighest=newProb;}

                   // if (nextFrame.has_cell(dstState)==false) {
                    destCell=&(nextFrame.insert_cell(dstState));
                    //}
                    
                   // destCell=&(nextFrame.get_cell_by_state(dstState));
                    
                    if (destCell->get_log_prob()<newProb){destCell->assign(newProb,dstWordTreeIdx);}
                }
            }

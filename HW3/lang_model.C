...
void LangModel::count_sentence_ngrams(const vector<int>& wordList)
    ...
    for (int n=1; n<=m_n; n++){
        for (int idx=m_n-n; idx<=wordCnt-n; idx++){
            vector <int> ngram(n);

            for (int i=0; i<=n-1 ; i++){
                ngram[i]=wordList[idx+i];
            }
    ...
    for (int n=0; n<m_n; n++){
        for (int idx=m_n-n-1; idx<=wordCnt-n-1; idx++){
            vector <int> ngram(n);
...
double LangModel::get_prob_plus_delta(const vector<int>& ngram) const
    ...
    //
    double retProb1=(m_predCounts.get_count(ngram)+m_delta);
    vector <int> temp;
    temp=ngram;
    temp.pop_back();
    double retProb2=(m_histCounts.get_count(temp)+m_delta*vocSize);
    retProb=retProb1/retProb2;
...
double LangModel::get_prob_witten_bell(const vector<int>& ngram) const
    ...
    //  END_LAB
    //
    vector <int> epsilon(0);
    vector <int> wi(1);
    wi[0]=ngram[m_n-1];
    double MLE_wi=1.0*m_predCounts.get_count(wi)/m_histCounts.get_count(epsilon);
    retProb=1.0*m_histCounts.get_count(epsilon)/(m_histCounts.get_count(epsilon)+m_histOnePlusCounts.get_count(epsilon))*MLE_wi+
    1.0*m_histOnePlusCounts.get_count(epsilon)/(m_histCounts.get_count(epsilon)+m_histOnePlusCounts.get_count(epsilon))/vocSize;


if (m_n>=2){
    vector <int> ngram2(2);
    ngram2[0]=ngram[m_n-2];
    ngram2[1]=ngram[m_n-1];

    vector <int> temp2(1);
    temp2[0]=ngram2[0];

    if (m_histCounts.get_count(temp2)!=0){
    double MLE_bi=1.0*m_predCounts.get_count(ngram2)/m_histCounts.get_count(temp2);
    retProb=1.0*m_histCounts.get_count(temp2)/(m_histCounts.get_count(temp2)+m_histOnePlusCounts.get_count(temp2))*MLE_bi+
    1.0*m_histOnePlusCounts.get_count(temp2)/(m_histCounts.get_count(temp2)+m_histOnePlusCounts.get_count(temp2))*retProb;
    }
}
...
if (m_n==3){
    ...
    if (m_histCounts.get_count(temp)!=0){
    double MLE_tri=1.0*m_predCounts.get_count(ngram)/m_histCounts.get_count(temp);
    retProb=1.0*m_histCounts.get_count(temp)/(m_histCounts.get_count(temp)+m_histOnePlusCounts.get_count(temp))*MLE_tri+
    1.0*m_histOnePlusCounts.get_count(temp)/(m_histCounts.get_count(temp)+m_histOnePlusCounts.get_count(temp))*retProb;
    }
}

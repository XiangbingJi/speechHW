...
void LangModel::count_sentence_ngrams(const vector<int>& wordList)
   ...
   for (int m=m_n; m>0; m--) { 
  	 for (int i=m_n-m; i<wordCnt-m+1; i++) { //at 0 or m_n-1? end at wordcnt-1??
  	 	vector <int> ngram(m);
		for (int j=0; j<m_n; j++) { //j 1 to m_n+1?  greater than? m_n-1 to >=0

			ngram[j]=wordList[i+j];
		}
		
		int test=m_predCounts.incr_count(ngram);
   ...
   for (int m=m_n-1; m>=0; m--) { 
  	 for (int i=m_n-m-1; i<wordCnt-m; i++) { //at 0 or m_n-1? end at wordcnt-1??
    	 vector<int> ngram(m);
		for (int j=0; j<m; j++) { //j 1 to m_n+1?  greater than? m_n-1 to >=0
			ngram[j]=wordList[i+j];
		}

...
double compute_distance(const matrix<double>& matHyp,
    ...
    {
    double dist = 0.0;
    double minvalue(double a1,double a2);
    int n=matHyp.size1();
    int m=matTempl.size1();
    int columns=matHyp.size2();
    double DTW[n][m];
    double cost;
    for (int i=1;i<n;i++){
        DTW[i][0]=100000.0;
    ...
    for (int i=1;i<n;i++){
        for (int j=1;j<m;j++){
            ...
            for (int k=0;k<columns;k++){
                cost+=pow((matHyp(i,k)-matTempl(j,k)),2);               
            }
            cost=sqrt(cost);

            DTW[i][j]=cost+minvalue(minvalue(DTW[i-1][j],DTW[i][j-1]),DTW[i-1][j-1]);
        }
    }
    return DTW[n-1][m-1];
    //  BEGIN_LAB

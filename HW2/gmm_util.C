...
double GmmStats::add_gmm_count(unsigned gmmIdx, double posterior,
    ...
    for (int dim=0;dim<=dimCnt-1;dim++){
        m_gaussStats1(gaussIdx,dim)+=posterior*feats[dim];
        m_gaussStats2(gaussIdx,dim)+=posterior*pow(feats[dim],2);
    }
...
void GmmStats::reestimate() const
    ...
    for (int idx=0;idx<=gaussCnt-1;idx++){
        for (int dim=0;dim<=dimCnt-1;dim++){
            double mu=m_gaussStats1(idx,dim)/m_gaussCounts[idx];
            m_gmmSet.set_gaussian_mean(idx, dim, mu);
            double sigma= (m_gaussStats2(idx,dim)-2*mu*m_gaussStats1(idx,dim)+m_gaussCounts[idx]*mu*mu)/m_gaussCounts[idx];
            m_gmmSet.set_gaussian_var(idx, dim, sigma);
            //m_gmmSet.set_gaussian_var(idx, dim, m_gaussStats1(idx,dim)/m_gaussCounts[idx]-pow(m_gaussStats1(idx,dim)/m_gaussCounts[idx],2));
        }
    }

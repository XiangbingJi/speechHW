...
double viterbi(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    chart(0,graph.get_start_state()).assign(0,-1);
    
    
    for (int f=0; f<frmCnt; f++){
        for (int s=0; s<=stateCnt-1;s++){
            int arc_Number=graph.get_arc_count(s);
            int current_arc_id=graph.get_first_arc_id(s);
            for (int idx=0; idx<arc_Number; idx++){
                Arc arc;
                int old_arc_id=current_arc_id;
                current_arc_id=graph.get_arc(current_arc_id,arc);
                int gmmId=arc.get_gmm();
                int dest=arc.get_dst_state(); 
                double temp=arc.get_log_prob()+gmmProbs(f,gmmId)+chart(f,s).get_log_prob();
                if (chart(f+1,dest).get_log_prob()<temp){
                    chart(f+1,dest).assign(temp,old_arc_id);
                }
            }
        }
    }
